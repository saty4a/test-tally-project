...
import { addOpeningAmount } from "../openingAmountApiCalls/data";
  ...
  export const AddExpenses = ({previousTotal, setIsAdded} : {previousTotal: number; setIsAdded: Function;}) => {
    ...
    const addExpenditureData = async () => {
      ...
      for (let index = 1; index < values.length; index++) {
        if(amountData.openingAmount !== values[index] && amountData.total !== values[index] ){
      ...
      <>
        ...
        >
          <ModalContent>
            ...
            <ModalBody>
              <StocksOptions amountData={amountData} updateDailyExpenditure={updateDailyExpenditure} type={type} />
              <p>Total Expenditure: {amountData.total}</p>
            </ModalBody>
            <ModalFooter>
  ...
  const StocksOptions = ({amountData, updateDailyExpenditure, type }: stockData) => {
    ...
    return (
      <div className="grid gap-4">
        <Input
          ...
          label="date"
          variant="bordered"
          onChange={(e) => { type.current = "date", updateDailyExpenditure(e.target.value)}}
        />
        <Input
          isClearable
          required
          ...
          min={0}
          onChange={(e) => { type.current = "openingAmount", updateDailyExpenditure(parseInt(e.target.value))}}
        />
