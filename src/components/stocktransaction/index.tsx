      ...
      try {
        ...
        if (res?.status === 200) {
          ...
          const totalAmount =
            ...
            res?.data?.data?.soldAmount +
            +res.data?.data?.loadAmount;
          await addOpeningAmount(previousAmount.id, totalAmount);
          setIsAdded(true);
        }
      } catch (error) {
        console.error(error);
          ...
          <TableBody items={items} emptyContent={"No data to display"}>
            {(item) => (
              <TableRow key={item.id}>
                ...
                <TableCell>{item.loadAmount}</TableCell>
                <TableCell>{item.soldCylinders}</TableCell>
