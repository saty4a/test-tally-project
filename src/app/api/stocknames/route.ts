...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!isValidateStocNamesData(stockNames)) {
      return new Response(
        JSON.stringify({ error: "Invalid stocknames data" }),
        { status: 400 }
      );
    }
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({
      error: "Error fetching stocknames",
      status: 500,
    });
  }
}
...
function isValidateStocNamesData(data: StockNames) {
  return (
    data.P1 && data.P2 && data.P3 && data.P4 && data.P5 && data.P6 && data.P7
  );
}
